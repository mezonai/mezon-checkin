...
func (dm *DMManager) SendDM(channelID, userID string, content models.ChannelMessageContent) error {
	return dm.SendDMWithContext(context.Background(), channelID, userID, content)
}

func (dm *DMManager) SendDMWithContext(ctx context.Context, channelID, userID string, content models.ChannelMessageContent) error {
...
func (dm *DMManager) buildDMEnvelope(channelID string, content models.ChannelMessageContent) (*rtapi.Envelope, error) {
...
func (dm *DMManager) sendDMMessage(ctx context.Context, envelope *rtapi.Envelope, channelID, userID string) error {
	// Check context
	select {
	case <-ctx.Done():
		return ctx.Err()
	...
	if ack := response.GetChannelMessageAck(); ack != nil {
		log.Printf("   Message ID: %s", ack.MessageId)
		log.Printf("   Create Time: %s", ack.CreateTime)
		return nil
	}
...
func (dm *DMManager) logSendDM(channelID, userID string) {
